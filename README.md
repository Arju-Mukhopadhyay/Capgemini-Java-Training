# 📘 Capgemini Java Training Progress

This repository documents the **day-wise syllabus and activities** covered during the **Java, DSA, SQL, Testing, DevOps, and Spring training program** at Capgemini.

---

# 🗓️ Session 1: Core Java Foundations

## 📅 07.01.2026 — Day 0
- Syllabus and training structure
- Test patterns
- Introduction to Java
- Tokens
- Eclipse for Java Enterprise
- Biometric registration

---

## 📅 08.01.2026 — Day 1
- Eclipse configuration (Workspace → Java Project → JRE 21)
- How Java runs a program
- Class naming conventions (CamelCase)
- Tokens
- Literals and Variables
- Type conversion
- Operators
- Operator precedence  
  https://www.cs.bilkent.edu.tr/~guvenir/courses/CS101/op_precedence.html

---

## 📅 09.01.2026 — Day 2 (Friday)
- Ternary operator
- Increment & decrement operators
- Logical operators
- Loops
- **Homework:** Write a Java program to convert decimal to binary

---

## 📅 10.01.2026 — Day 3 (Saturday)
- Pattern printing
- Methods / Functions
- Recursion
- Quick sort, merge sort, linked list
- Test

---

## 📅 13.01.2026 — Day 4 (Tuesday)
- Methods and functions (completed)
- Recursion
- Pattern printing
- Concepts of objects, methods, and classes
- Access specifiers
- Static vs non-static methods
- Basic syllabus shared

---

## 📅 14.01.2026 — Day 5 (Wednesday)
- Constructors (default, parameterized, non-parameterized)
- Arrays and 2D arrays
- Classes
- Objects

---

## 📅 15.01.2026 — Day 6 (Thursday)
- Advanced arrays (sum of array)
- Strings
- Regex
- StringBuffer
- StringBuilder
- Practice problems

---

## 📅 16.01.2026 — Day 7 (Friday)
- Objects & Classes (Advanced)
- Encapsulation
- Scanner class
- Subarrays
- Practice problems (sliding window, two pointers)
- Object connections

---

## 📅 17.01.2026 — Day 8 (Saturday)
- Association
  - Aggregation
  - Composition
- Inheritance
- Weekly assessment

---

## 📅 19.01.2026 — Day 9 (Monday)
- Revision of previous topics
- Advanced inheritance
- Types of inheritance

---

## 📅 20.01.2026 — Day 10 (Tuesday)
- Upcasting & Downcasting
- Polymorphism

---

## 📅 21.01.2026 — Day 11 (Wednesday)
- Abstract classes
- Interfaces
- Exception handling

---

## 📅 22.01.2026 — Day 12 (Thursday)
- Error handling techniques
- Wrapper classes
- Collections framework

---

## 📅 24.01.2026 — Day 13 (Saturday)
- Collections (Advanced)
- LinkedList
- Stack
- HashMap & hashing
- Weekly Test (4 coding + 60 MCQs)

---

## 📅 27.01.2026 — Day 14 (Tuesday)
- ArrayList programs
- HashSet
- LinkedHashSet
- TreeSet
- Method overriding problems

---

# ✅ Session 1 Complete

---

# 🗓️ Session 2: DSA, SQL & Advanced Java

## 📅 28.01.2026 — Day 15
- Introduction to Data Structures & Algorithms

---

## 📅 29.01.2026 — Day 16
- Communication & Soft Skills

---

## 📅 30.01.2026 — Day 17
- DSA: Arrays and Strings

---

## 📅 31.01.2026 — Day 18
- Collections revision
- Queue, Stack, Vector
- Weekly test (2 coding problems)

---

## 📅 02.02.2026 — Day 19
- Searching techniques
- Merge sort & quick sort

---

## 📅 03.02.2026 — Day 20
- Problem solving
- Dynamic Programming (Started)

---

## 📅 04.02.2026 — Day 21
- Knapsack problem
- Greedy algorithms

---

## 📅 05.02.2026 — Day 22
- Fractional knapsack
- Hashing (Anagram program)
- Heaps
- Huffman algorithm

---

## 📅 06.02.2026 — Day 23
- Mock test

---

## 📅 07.02.2026 — Day 24
- Mock test discussion
- DP & recursion revision
- Weekly programming test

---

## 📅 09.02.2026 — Day 25
- Self practice
- Capgemini interaction

---

## 📅 10.02.2026 — Day 26
- DB, DBMS, RDBMS introduction

---

## 📅 11.02.2026 — Day 27
- English & Soft Skills

---

## 📅 12.02.2026 — Day 28
- SQL continuation
- DDL, DML, DCL, TCL

---

## 📅 13.02.2026 — Day 29
- WHERE, GROUP BY, HAVING
- Joins

---

## 📅 14.02.2026 — Day 30
- Inner, Natural, Outer Joins
- Triggers
- Weekly SQL Test

---

## 📅 16.02.2026 — Day 31
- DSA revision
- CodeChef & HackerRank

---

## 📅 17.02.2026 — Day 32
- Binary search
- Heaps & priority queue
- Advanced Java concepts

---

## 📅 18.02.2026 — Day 33
- Generic classes
- Lambda expressions

---

## 📅 19.02.2026 — Day 34
- Stream API
- Comparator
- Threads (Started)

---

## 📅 20.02.2026 — Day 35
- Threads & synchronization
- File handling

---

## 📅 21.02.2026 — Day 36
- Top Brains problems
- Virtual Judge (72 questions)
- Weekly Test (DSA + Java)

---

## 📅 23.02.2026 — Day 37
- Self practice
- HackerRank M1 Test

---

# ✅ Session 2 Complete

---

# 🗓️ Session 3: Testing, JDBC, DevOps & Spring

## 📅 24.02.2026 — Day 38
- JUnit testing
- Maven
- Parameterized & Non-parameterized tests
- Testing types (Alpha, Beta, Black box, White box)

---

## 📅 25.02.2026 — Day 39
- English & Communication

---

## 📅 26.02.2026 — Day 40
- JDBC
- MySQL connection
- CRUD operations

---

## 📅 27.02.2026 — Day 41
- Task, TaskDAO, TaskDTO

---

## 📅 28.02.2026 — Day 42
- Weekly Test (Java & SQL)
- Mock & Final coding tests

---

## 📅 02.03.2026 — Day 43
- Added comment lines for next steps
- Task, TaskDAO, TaskDTO
- User, UserDAO, UserDTO
- Updated pom.xml, main.java and persistence.xml
- Many-to-many transactions

---

## 📅 05.03.2026 — Day 44
- Hibernate framework
- JDBC basics
- Entity and table mapping
- Relationship mapping
- Customer & Customer Details project

---

## 📅 06.03.2026 — Day 45
- DevOps Introduction (Theory)
- Waterfall Model
- Agile Model
- DevOps overview

---

## 📅 07.03.2026 — Day 46
- GenAI Online Class
- Version Control System
- Git & GitHub
- Jenkins installation

---

## 📅 09.03.2026 — Day 47
- Jenkins tutorial
- CI/CD pipeline
- Created and deployed projects in Jenkins

---

## 📅 10.03.2026 — Day 48
- Power skills
- Email writing

---

## 📅 11.03.2026 — Day 49
- Spring Framework (Theory)
- Spring Modules and Architecture
- Spring configuration
- Hands-on coding practice

---

## 📅 12.03.2026 — Day 50
- Spring Boot introduction
- Spring Boot architecture
- Design patterns
- Create & configure Spring Boot project
- Constructor Injection & Setter Injection
- Installed Apache Tomcat and Postman

---

## 📅 13.03.2026 — Day 51
- *(To be updated)*

---
